...
4. **Repository Test Stack**:
   ...
   - If the repository has not declared these details, report the gap explicitly, suggest running the pack's init workflow (`/superpowers-extended:init` as a plugin; `commands/init.md` otherwise), and use the strongest available local validation without inventing fake commands

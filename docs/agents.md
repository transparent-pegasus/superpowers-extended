...
Agent definitions live in a single tree at `agents/<name>.md`, exposed to each platform through the pack's plugin manifests.
...
| Cursor | Agents ship via the plugin manifest; otherwise reference `agents/<name>.md` in the prompt. |
| Codex | Plugins cannot ship agent personas — reference `agents/<name>.md` in the prompt; the persona file describes the contract the agent must follow. |
